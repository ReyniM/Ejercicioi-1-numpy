...
def summary(a: np.ndarray) -> tuple[float, float, float, float]:
    ...
    result = np.array((np.min(a), np.max(a), np.mean(a), np.std(a)))
    array_result = tuple(result.astype(float))
    return array_result
...
def ith(temperature: np.ndarray, humidity: np.ndarray) -> ndarray[Any, dtype[floating[Any]]] | tuple[
    str, str, str, str, str]:
    """
    ...
    :param temperature:  np.ndarray
        temperatures collected by the sensor in a gridf
    ...
    if temperature.shape != humidity.shape:
        raise ValueError('Shape of data sensors must be the same. Temperature: (2, 2) != humidity: (2, 3)')
    else:
        ith = np.around(((0.8 * temperature) + (humidity / 100) * (temperature - 14.3)) + 46.4, 0)
        return ith
...
def isStress(ith: np.ndarray) -> np.ndarray:
    ...
    a = np.array(ith)
    result = np.where(a > 78, True, False)
    return result
